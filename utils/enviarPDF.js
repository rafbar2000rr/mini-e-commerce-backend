...
//-------------------------------------------------------------------------------
// 📌 Genera un PDF profesional con logo, estado, fecha y precios correctos
//-------------------------------------------------------------------------------
async function generarPDF(orden) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // 🩷 Encabezado elegante con el nombre de la tienda
      doc
        .fillColor("#D63384")
        .font("Helvetica-Bold")
        .fontSize(26)
      ...
      const fechaOrden =
        orden.fecha || orden.fechaOrden || orden.createdAt || Date.now();

      doc.fillColor("black").fontSize(12);
      doc.text(`ID de la Orden: ${orden._id}`);
      doc.text(
        `Fecha: ${new Date(fechaOrden).toLocaleString("es-PE", {
          dateStyle: "full",
          timeStyle: "short",
        })}`
      );
      ...
      doc.font("Helvetica-Bold").fillColor(colorEstado).text(`Estado: ${estado}`);
      doc.moveDown(0.5);

      // Total
      doc.font("Helvetica-Bold").fillColor("#D63384").text(
      ...
      doc.text(
        `Código Postal: ${orden.datosCliente?.codigoPostal || "No disponible"}`
      );
      doc.moveDown(1);
      ...
      for (const p of orden.productos) {
        ...
        const precioPagado =
          p.precioPagado ?? p.precio ?? p.precioUnitario ?? 0;
        const cantidad = p.cantidad || 1;
        const subtotal = (precioPagado * cantidad).toFixed(2);
        ...
        doc
          ...
          .fontSize(12)
          .text(
            `$${precioPagado.toFixed(2)} x ${cantidad} = $${subtotal}`,
            60,
            yInicio + 18,
            { width: 350 }
          );
        ...
        if (p.imagen) {
          try {
            const response = await axios.get(p.imagen, {
              responseType: "arraybuffer",
            });
            const buffer = Buffer.from(response.data, "binary");
            doc.image(buffer, 420, yInicio, {
              width: 80,
              height: 80,
              fit: [80, 80],
            });
      ...
      // Total final
      doc.moveDown(1);
      doc.font("Helvetica-Bold").fontSize(14).fillColor("#D63384");
      doc.text(`TOTAL FINAL: $${(orden.total ?? 0).toFixed(2)}`, {
        align: "right",
      });
